...
function FamilyNameSheet({ name, onCancel, onSave }: { name: string; onCancel: () => void; onSave: (name: string) => void }) {
  ...
  return (
    <div className="profile-edit-sheet" role="presentation">
      <button className="profile-edit-sheet__backdrop" type="button" aria-label="Avbryt" onClick={onCancel} />
      <section className="profile-edit-sheet__panel" role="dialog" aria-modal="true" aria-labelledby="family-name-title">
        ...
        <label className="profile-edit-sheet__field">
          ...
          <input autoFocus value={value} onChange={(event) => { setValue(event.target.value); setError(""); }} />
        </label>
...
function MemberEditSheet({ member, onCancel, onSave }: { member: FamilyMember; onCancel: () => void; onSave: (displayName: string, role: RoleChoice) => void }) {
  ...
  return (
    <div className="profile-edit-sheet" role="presentation">
      ...
      <section className="profile-edit-sheet__panel" role="dialog" aria-modal="true" aria-labelledby="member-edit-title">
        ...
        <label className="profile-edit-sheet__field">
          <span>Navn</span>
          <input autoFocus value={displayName} onChange={(event) => { setDisplayName(event.target.value); setError(""); }} />
        </label>
        <label className="profile-edit-sheet__field">
          <span>Rolle</span>
...
function InviteSheet({ onCancel, onSend }: { onCancel: () => void; onSend: (email: string, role: RoleChoice) => void }) {
  ...
  return (
    <div className="profile-edit-sheet" role="presentation">
      ...
      <section className="profile-edit-sheet__panel" role="dialog" aria-modal="true" aria-labelledby="invite-title">
        ...
        <label className="profile-edit-sheet__field">
          <span>E-post</span>
          <input autoFocus inputMode="email" type="email" value={email} onChange={(event) => { setEmail(event.target.value); setError(""); }} />
        </label>
        <label className="profile-edit-sheet__field">
          <span>Rolle</span>
          <select value={role} onChange={(event) => setRole(event.target.value as RoleChoice)}>
        ...
        <div className="profile-edit-sheet__actions">
          <button className="profile-edit-sheet__button profile-edit-sheet__button--secondary" type="button" onClick={onCancel}>Avbryt</button>
          <button className="profile-edit-sheet__button profile-edit-sheet__button--primary" type="button" onClick={handleSend}>Send invitasjon</button>
        </div>
      </section>
    </div>
  );
...
function RemoveSheet({ member, isSelf, onCancel, onRemove }: { member: FamilyMember; isSelf: boolean; onCancel: () => void; onRemove: () => void }) {
  return (
    <div className="profile-edit-sheet" role="presentation">
      <button className="profile-edit-sheet__backdrop" type="button" aria-label="Avbryt" onClick={onCancel} />
      <section className="profile-edit-sheet__panel profile-edit-sheet__panel--compact" role="dialog" aria-modal="true" aria-labelledby="remove-member-title">
        ...
        <p className="profile-edit-sheet__placeholder-text">{isSelf ? "Dette er deg. Bekreft bare hvis du vil fjerne din egen tilgang." : `${member.displayName} fjernes fra familien.`}</p>
        <div className="profile-edit-sheet__actions">
          <button className="profile-edit-sheet__button profile-edit-sheet__button--secondary" type="button" onClick={onCancel}>Avbryt</button>
          <button className="profile-edit-sheet__button profile-edit-sheet__button--danger" type="button" onClick={onRemove}>Fjern</button>
        </div>
      </section>
    </div>
  );
...
export function FamilySettingsClient() {
  ...
  async function saveFamilyName(name: string) {
    if (!family) return;
    try {
      const updated = await updateFamily(family.id, { name });
      setFamily(updated);
      setSheet(null);
    } catch {
  ...
  async function saveMember(member: FamilyMember, displayName: string, role: RoleChoice) {
    if (!family) return;
    try {
      const updated = await updateFamilyMember(family.id, member.id, { displayName, role: choiceToApiRole(role) });
  ...
  async function resendInvite(invitation: FamilyInvitation) {
    if (!family) return;
    try {
      const response = await resendFamilyInvitation(family.id, invitation.id);
  ...
  async function revokeInvite(invitation: FamilyInvitation) {
    if (!family) return;
    try {
      const updated = await revokeFamilyInvitation(family.id, invitation.id);
    ...
    return (
      <FamilySettingsShell>
        <div className="settings-placeholder-card family-settings__empty">
          ...
          <button className="family-settings__pill-button" type="button" onClick={loadData}>Prøv igjen</button>
        </div>
      </FamilySettingsShell>
    );
  }
  ...
  return (
    <FamilySettingsShell>
                  ...
                  <span className="family-settings-menu">
                    ...
                    {openMemberMenu === member.id ? (
                      <span className="family-settings-menu__panel">
                        <button type="button" onClick={() => { setSheet({ type: "edit-member", member }); setOpenMemberMenu(null); }}>Rediger medlem</button>
                        <button type="button" disabled={cannotRemoveLastAdmin} onClick={() => { setSheet({ type: "remove-member", member }); setOpenMemberMenu(null); }}>Fjern medlem</button>
                      </span>
                    ) : null}
      ...
      {isAdmin ? (
        <SettingsSection title="Invitasjoner">
          <SettingsCard>
            <div className="family-settings__section-head">
              <h2>Invitasjoner</h2>
              <button className="family-settings__small-button" type="button" onClick={() => setSheet({ type: "invite" })}>Inviter familiemedlem</button>
            </div>
            {invitations.length ? invitations.map((invitation) => (
              <div className="family-settings-invite" key={invitation.id}>
                ...
                {invitation.status === "pending" ? (
                  <span className="family-settings-menu">
                    <button className="family-settings__icon-button" type="button" aria-label={`Handlinger for ${invitation.invitedEmail}`} onClick={() => setOpenInviteMenu(openInviteMenu === invitation.id ? null : invitation.id)}><MoreHorizontal aria-hidden="true" /></button>
                    {openInviteMenu === invitation.id ? (
                      <span className="family-settings-menu__panel">
                        <button type="button" onClick={() => void resendInvite(invitation)}>Send på nytt</button>
                        <button type="button" onClick={() => void revokeInvite(invitation)}>Trekk tilbake</button>
                      </span>
                    ) : null}
                  </span>
                ) : null}
      ...
      {sheet?.type === "family-name" ? <FamilyNameSheet name={family.name} onCancel={() => setSheet(null)} onSave={saveFamilyName} /> : null}
      {sheet?.type === "edit-member" ? <MemberEditSheet member={sheet.member} onCancel={() => setSheet(null)} onSave={(displayName, role) => void saveMember(sheet.member, displayName, role)} /> : null}
      {sheet?.type === "remove-member" ? <RemoveSheet member={sheet.member} isSelf={sheet.member.id === currentMembership?.id} onCancel={() => setSheet(null)} onRemove={() => void removeMember(sheet.member)} /> : null}
      {sheet?.type === "invite" ? <InviteSheet onCancel={() => setSheet(null)} onSend={(email, role) => void sendInvite(email, role)} /> : null}
    </FamilySettingsShell>
  );
}
