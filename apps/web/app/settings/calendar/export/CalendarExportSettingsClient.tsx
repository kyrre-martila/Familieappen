...
import { ChevronLeft, Copy, Link as LinkIcon, RefreshCw, ShieldAlert } from "lucide-react";
...
export function CalendarExportSettingsClient() {
  ...
  const loadCalendarExportSettings = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(getApiMessage(error, "Kunne ikke hente kalenderabonnementet akkurat nå"));
    }
  }, [familyId]);
  ...
  return (
    <main className="settings-shell settings-shell--detail calendar-export-settings" aria-label="Kalenderabonnement">
      <Link className="settings-back-link" href="/settings/calendar" aria-label="Tilbake til kalenderinnstillinger">
        <ChevronLeft aria-hidden="true" />
      </Link>
      <header className="settings-hero settings-hero--detail">
        <h1>Kalenderabonnement</h1>
        <p>Del FamilieAppen-kalenderen med Apple Kalender, Google Kalender eller Outlook.</p>
      </header>
      ...
      <section className="calendar-settings-section" aria-labelledby="calendar-export-title">
        ...
        <Card className="calendar-export-card">
          <div className="calendar-export-card__intro">
            ...
            <div><h3 id="calendar-export-title">Privat kalenderabonnement</h3><p>Lag en privat kalenderlenke som kan abonneres på i Apple Kalender, Google Kalender eller Outlook.</p></div>
            <Badge tone={feed?.enabled ? "success" : "neutral"}>{feed?.enabled ? "Aktiv" : "Inaktiv"}</Badge>
          </div>
          <label className="settings-toggle-row" htmlFor="calendar-export-enabled"><span><span className="settings-label">Aktiver kalenderabonnement</span><span className="settings-help">Kalenderklienter kan lese lenken uten innlogging.</span></span><input id="calendar-export-enabled" className="settings-toggle" type="checkbox" checked={Boolean(feed?.enabled)} disabled={!feed || busyId === "feed"} onChange={(event) => void updateExportFeed({ enabled: event.target.checked })} /></label>
          <div className="calendar-export-url" aria-label="Privat kalenderlenke">
            ...
            <div className="calendar-export-url__actions"><Button variant="secondary" onClick={copyExportLink} disabled={!feed?.enabled}><Copy aria-hidden="true" size={18} /> Kopier lenke</Button><Button variant="secondary" onClick={regenerateExportLink} disabled={!feed || busyId === "feed"}><RefreshCw aria-hidden="true" size={18} /> Regenerer lenke</Button></div>
          ...
          <div className="settings-fieldset" role="group" aria-labelledby="calendar-export-content-label"><div className="settings-fieldset__header"><span className="settings-label" id="calendar-export-content-label">Innhold i eksporten</span><span className="settings-help">Velg hva ICS-feeden skal inkludere.</span></div><label className="settings-toggle-row" htmlFor="export-include-events"><span className="settings-label">Kalenderhendelser</span><input id="export-include-events" className="settings-toggle" type="checkbox" checked={Boolean(feed?.includeEvents)} onChange={(event) => void updateExportFeed({ includeEvents: event.target.checked })} /></label><label className="settings-toggle-row" htmlFor="export-include-meals"><span className="settings-label">Middager</span><input id="export-include-meals" className="settings-toggle" type="checkbox" checked={Boolean(feed?.includeMeals)} onChange={(event) => void updateExportFeed({ includeMeals: event.target.checked })} /></label><label className="settings-toggle-row" htmlFor="export-include-reminders"><span className="settings-label">Husk-påminnelser</span><input id="export-include-reminders" className="settings-toggle" type="checkbox" checked={Boolean(feed?.includeReminders)} onChange={(event) => void updateExportFeed({ includeReminders: event.target.checked })} /></label><label className="settings-toggle-row" htmlFor="export-include-school-week"><span className="settings-label">Skoleuka-påminnelser</span><input id="export-include-school-week" className="settings-toggle" type="checkbox" checked={Boolean(feed?.includeSchoolWeekReminders)} onChange={(event) => void updateExportFeed({ includeSchoolWeekReminders: event.target.checked })} /></label></div>
          <div className="settings-fieldset" role="group" aria-labelledby="calendar-export-scope-label"><div className="settings-fieldset__header"><span className="settings-label" id="calendar-export-scope-label">Hvem kalenderen gjelder</span><span className="settings-help">Hele familien er støttet nå. Avgrensning er strukturert for videre utvidelse.</span></div><div className="segmented-control segmented-control--export" role="radiogroup" aria-labelledby="calendar-export-scope-label">{exportScopeOptions.map((option) => <label className="segmented-control__option" key={option.value}><input type="radio" name="calendarExportScope" value={option.value} checked={feed?.scope === option.value} onChange={() => void updateExportFeed({ scope: option.value })} /><span>{option.label}</span></label>)}</div>{feed?.scope === "selectedParticipant" ? <label className="settings-select-row" htmlFor="export-selected-participant"><span><span className="settings-label">Valgt familiemedlem</span><span className="settings-help">Bare kalenderhendelser knyttet til dette familiemedlemmet tas med.</span></span><select id="export-selected-participant" value={feed.selectedFamilyMemberId ?? ""} onChange={(event) => void updateExportFeed({ selectedFamilyMemberId: event.target.value || null })}><option value="">Velg familiemedlem</option>{familyMembers.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label> : null}</div>
        </Card>
      </section>
    </main>
  );
