...
export default function WasteCollectionPage() {
  const access = useFamilyAccess(); const familyId = access.familyContext?.activeFamilyId ?? null;
  const [subscription,setSubscription] = useState<WasteSubscription|null>(null); const [events,setEvents] = useState<WasteEvent[]>([]);
  const [state,setState] = useState<"loading"|"ready"|"no-address"|"error">("loading");
  useEffect(() => { if (!familyId) return; let live=true; const from=osloToday();
    getWasteSubscription(familyId).then(async sub => { const items=await getWasteEvents(familyId,from,plusDays(from,90)); if(live){setSubscription(sub);setEvents(items);setState("ready");} }).catch(error => { if(live)setState(error instanceof ApiError&&error.status===404?"no-address":"error"); }); return()=>{live=false};
  },[familyId]);
  return <AppShell title="Renovasjon"><PageContainer><section className="waste-page"><SectionHeader eyebrow="Hjemmet" title="Renovasjon" />
    ...
    {state==="ready"&&subscription?<><Card><p className="waste-address-label">Familiens adresse</p><strong>{subscription.address.streetName} {subscription.address.houseNumber}{subscription.address.houseLetter}<br/>{subscription.address.postalCode} {subscription.address.postalPlace}</strong>{subscription.lastSyncStatus==="error"?<p className="waste-warning">Siste synkronisering feilet. Vi prøver igjen automatisk.</p>:null}</Card>
      <Card><h2>Neste tømminger</h2>{events.length?<ul className="waste-events">{events.map(event=><li key={event.id}><strong>{event.name}</strong><time dateTime={event.collectionDate}>{displayDate(event.collectionDate)}</time></li>)}</ul>:<p>Vi har ingen tilgjengelige tømmedata ennå. Dette betyr ikke nødvendigvis at det ikke er planlagt tømming.</p>}</Card></>:null}
