...
export interface HealthPlanNotificationRecipient { familyMemberId: string; familyMember: FamilyMember }
export interface HealthPlan { id: string; familyMemberId: string | null; subjectDisplayName: string; name: string; description: string | null; status: HealthPlanStatus; activeLevelId: string | null; activeStepId: string | null; familyMember: FamilyMember | null; activeLevel?: Pick<HealthPlanLevel, "id" | "levelIndex" | "name"> | null; activeStep?: Pick<HealthPlanStep, "id" | "stepOrder" | "name"> | null; levels?: HealthPlanLevel[]; notificationRecipients?: HealthPlanNotificationRecipient[] }
export interface HealthPlanOccurrence { id: string; healthPlanId: string; scheduledAt: string; originalScheduledAt: string; actionTitle: string; actionInstruction: string | null; status: HealthPlanOccurrenceStatus; completedAt: string | null; occurrenceLevel: Pick<HealthPlanLevel, "levelIndex" | "name"> | null; occurrenceStep: Pick<HealthPlanStep, "stepOrder" | "name"> | null; healthPlan: Pick<HealthPlan, "id" | "name" | "status" | "subjectDisplayName"> & { familyMember: Pick<FamilyMember, "id" | "displayName"> | null } }
export type HealthPlanHistoryType = "CREATED" | "PAUSED" | "RESUMED" | "LEVEL_INCREASED" | "LEVEL_DECREASED" | "STEP_ADVANCED" | "STATUS_CHANGED" | "DEFINITION_UPDATED";
export interface HealthPlanHistory { id: string; healthPlanId: string; type: HealthPlanHistoryType; actorDisplayName: string | null; metadata: Record<string, unknown> | null; occurredAt: string }
export type HealthPlanNoteTargetContext = { kind: "OCCURRENCE"; actionTitle: string; scheduledAt: string; levelIndex: number; stepOrder: number } | { kind: "ACTION"; actionTitle: string; localTime: string; levelIndex: number; stepOrder: number };
export interface HealthPlanNote { id: string; healthPlanId: string; occurrenceId: string | null; sourceActionId: string | null; text: string; createdAt: string; authorDisplayName: string | null; targetContext: HealthPlanNoteTargetContext | null }
export interface HealthPlanLog { history: HealthPlanHistory[]; notes: HealthPlanNote[] }
export interface CreateHealthPlanInput { familyMemberId: string; name: string; description?: string | null; notificationRecipientIds?: string[]; levels: Array<{ levelIndex: number; name?: string; steps: Array<{ stepOrder: number; name?: string; durationDays?: number | null; autoAdvance?: boolean; schedules: Array<{ recurrenceType: "DAILY" | "WEEKDAYS" | "INTERVAL_DAYS"; localTime: string; timezone?: string; weekdays?: number[]; intervalDays?: number; anchorDate?: string; actions: Array<{ title: string; instruction?: string | null; sortOrder: number }> }> }> }> }
...
export type NotificationPreferenceUpdate = Partial<Pick<NotificationPreferences, "shoppingEnabled" | "calendarEnabled" | "remindersEnabled" | "tasksEnabled" | "mealsEnabled" | "wishlistEnabled" | "systemEnabled" | "healthPlansEnabled">>;
...
export function getHealthPlanLog(familyId: string, id: string, signal?: AbortSignal): Promise<HealthPlanLog> { return apiRequest<HealthPlanLog>(`/health-plans/${encodeURIComponent(id)}/log`, { familyId, signal }); }
...
export function updateHealthPlanNotificationRecipients(familyId: string, id: string, familyMemberIds: string[], signal?: AbortSignal): Promise<HealthPlan> { return apiRequest<HealthPlan>(`/health-plans/${encodeURIComponent(id)}/notification-recipients`, { method: "PATCH", body: { familyMemberIds }, familyId, signal }); }
...
export function addHealthPlanNote(familyId: string, healthPlanId: string, input: { text: string; occurrenceId?: string; sourceActionId?: string }, signal?: AbortSignal): Promise<unknown> { return apiRequest(`/health-plans/${encodeURIComponent(healthPlanId)}/notes`, { method: "POST", body: input, familyId, signal }); }
