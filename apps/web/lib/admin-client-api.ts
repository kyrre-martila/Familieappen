import { AdminApiError, safeAdminErrorMessage, type AdminDashboard, type AdminManagedUserDetail, type AdminStatistics, type AdminUser, type AdminUserListResponse, type AdminUserStatusUpdate, type AdvertisementListResponse, type Advertisement, type AdvertisementMutation, type AuditLogResponse, type AdminFamilySearchResponse, type AdminFamilyInviteCode, type AdminFamilyRole } from "./admin-shared";
...
export async function searchAdminFamilies(query: { search?: string; inviteCode?: string; page?: number; pageSize?: number } = {}): Promise<AdminFamilySearchResponse> { const q=qs(query); return adminClientRequest(`/admin/families${q ? `?${q}` : ""}`); }
export async function fetchAdminFamilyInviteCode(userId:string, familyId:string): Promise<AdminFamilyInviteCode> { return adminClientRequest(`/admin/users/${encodeURIComponent(userId)}/families/${encodeURIComponent(familyId)}/invite-code`); }
export async function moveAdminUserFamily(userId:string, body:{fromFamilyId:string; targetFamilyId:string; role:AdminFamilyRole; reason:string}): Promise<AdminManagedUserDetail> { return adminClientRequest(`/admin/users/${encodeURIComponent(userId)}/move-family`, { method:"POST", body }); }
export async function createAdminUserFamily(userId:string, body:{name:string; reason:string}): Promise<AdminManagedUserDetail> { return adminClientRequest(`/admin/users/${encodeURIComponent(userId)}/create-family`, { method:"POST", body }); }
