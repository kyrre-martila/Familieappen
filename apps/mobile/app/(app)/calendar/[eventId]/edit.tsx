...
export default function EditCalendarEventScreen() {
  const params = useLocalSearchParams<{ eventId?: string; occurrenceDate?: string; scope?: string }>();
  ...
  const scope = parseCalendarEventEditScope(params.scope);
  ...
  const loadedRouteError = validateCalendarEventEditRoute({ isRecurringOccurrence: details.event.isRecurringOccurrence, scope, occurrenceDate });
  if (loadedRouteError) return <Screen bottomInset="screen"><View style={styles.topbar}><Button title="Tilbake" variant="secondary" onPress={() => safeBack(eventId, occurrenceDate)} /></View><ErrorState title="Kan ikke åpne redigering" description={loadedRouteError} onRetry={() => safeBack(eventId, occurrenceDate)} /></Screen>;
  ...
  return <View style={styles.root}><View style={styles.topbar}><Button title="Tilbake" variant="secondary" onPress={() => safeBack(eventId, occurrenceDate)} disabled={updateMutation.saving} /><AppText variant="label">Kalender</AppText></View><CalendarEventForm title="Rediger hendelse" description={scope === "occurrence" ? "Oppdater bare denne forekomsten." : scope === "series" ? "Oppdater hele serien uten å endre gjentakelsesregelen." : "Oppdater denne kalenderhendelsen for familien."} form={form} onChange={update} onSubmit={() => updateMutation.updateEvent(form)} onCancel={() => safeBack(eventId, occurrenceDate)} submitting={updateMutation.saving} submitTitle="Lagre endringer" submittingTitle="Lagrer …" error={updateMutation.error} footer={scope === "occurrence" ? "Endringen gjelder kun valgt dato." : scope === "series" ? "Gjentakelsesregelen beholdes uendret." : undefined} /></View>;
}
