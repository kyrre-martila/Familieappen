...
import {
  ...
  reminderPickerDateToString,
  validateReminderForm,
  type ReminderForm as Form,
} from "../reminderForm";
type Props = {
  title: string;
  description: string;
...
export function ReminderForm({
  ...
  const [errors, setErrors] = useState<ReturnType<typeof validateReminderForm>>(
    {},
  );
  ...
  const submit = async () => {
    ...
    const next = validateReminderForm(form);
    setErrors(next);
        ...
        <Card style={styles.card}>
          ...
          <Field label="Tittel" error={errors.title}>
            <TextInput
              ...
              accessibilityHint="Hva skal huskes"
          ...
          <Field label="Notat">
            <TextInput
              ...
              accessibilityHint="Valgfritt notat"
              placeholder="Valgfritt notat"
              placeholderTextColor={theme.colors.placeholder}
              style={[styles.input, styles.note]}
            />
          </Field>
          <Field label="Dato" error={errors.date}>
            <Pick
              label={form.date}
              accessibilityLabel="Velg dato"
          ...
          <View style={styles.row}>
            <AppText variant="label">Varsling</AppText>
            <Switch
              value={form.reminderEnabled}
              onValueChange={(v) => update("reminderEnabled", v)}
              disabled={isSubmitting}
              accessibilityLabel="Varsling"
            />
          </View>
            ...
            <View style={styles.options}>
              {reminderMinuteOptions.map((option) => (
                <Pick
                  key={option.value}
                  label={option.label}
                  accessibilityLabel={`Varsling ${option.label}`}
                  onPress={() => update("reminderMinutesBefore", option.value)}
          ...
          <Field label="Ikon">
            <ScrollView
              ...
              accessibilityLabel="Velg ikon"
            >
              {reminderIcons.map((icon) => (
                <Pick
                  key={icon}
                  label={icon}
                  accessibilityLabel={`Ikon ${icon}`}
          ...
          <View style={styles.row}>
            <View>
              <AppText variant="label">Privat</AppText>
              <AppText variant="small" style={styles.muted}>
                Bare du kan se påminnelsen.
              </AppText>
            </View>
            <Switch
              value={form.isPrivate}
              onValueChange={(v) => update("isPrivate", v)}
              disabled={isSubmitting}
              accessibilityLabel="Privat påminnelse"
            />
          </View>
          <Field label="Ansvarlig" error={errors.memberIds}>
            <View style={styles.options}>
              <Pick
                label="Hele familien"
                accessibilityLabel="Hele familien"
                onPress={() => update("scope", "family")}
              ...
              {familyMembers.map((member) => (
                <Pick
                  ...
                  onPress={() => {
                    const ids = form.memberIds.includes(member.id)
          ...
          <Button
            ...
            accessibilityHint="Lagrer påminnelsen."
          />
          <Button
            title="Avbryt"
            variant="ghost"
      ...
      {picker ? (
        <DateTimePicker
          ...
          onChange={(event, date) => {
            ...
            if (picker === "date")
              update("date", reminderPickerDateToString(date));
            else
              update(
                "time",
...
function Pick({
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      disabled={disabled}
      style={[styles.pick, selected && styles.selected]}
    >
      <AppText style={selected && styles.selectedText}>{label}</AppText>
    </Pressable>
  );
}
const styles = StyleSheet.create({
  ...
  note: { minHeight: 104, textAlignVertical: "top" },
  row: {
    minHeight: 48,
    flexDirection: "row",
    alignItems: "center",
