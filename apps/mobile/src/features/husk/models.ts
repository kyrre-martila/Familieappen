...
export type ReminderViewModel = { id: string; title: string; icon: string; dateLabel: string; timeLabel: string | null; assigneeLabel: string; status: ReminderStatus; statusLabel: string; date: string | null };
export type HuskListViewModel = { id: string; title: string; icon: string; completedCount: number; totalCount: number; progressPercent: number; progressLabel: string };
...
function localDate(value: string) { return new Date(`${value.slice(0, 10)}T12:00:00`); }

export function getReminderStatus(date: string | null, today = todayString()): ReminderStatus {
...
export function formatReminderDate(date: string | null, today = todayString()) {
  ...
  if (value === tomorrow.toISOString().slice(0, 10)) return "I morgen";
...
export function mapReminderToViewModel(reminder: Reminder, today = todayString()): ReminderViewModel {
  const status = getReminderStatus(reminder.date, today);
  const statusLabel = ({ overdue: "Forfalt", today: "I dag", upcoming: "Kommende", unscheduled: "Uten dato" } as const)[status];
  const assigneeLabel = reminder.scope === "family" || reminder.memberIds.length === 0 ? "Hele familien" : reminder.audienceMembers.length === 1 ? reminder.audienceMembers[0].familyMember.displayName : `${reminder.memberIds.length} personer`;
