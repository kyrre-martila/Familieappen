...
import { getHuskLists, getHuskReminders, getSchoolWeekReminders, getTasks } from "../api";
...
import { mapTaskToViewModel, sortTasks } from "../taskModel";
...
export function useHusk(filter: ReminderFilter = "active", view: HuskView = "reminders", selectedWeekStart?: string) {
  ...
  const tasks = useMemo(() => sortTasks(tasksQuery.data ?? []).map((task) => mapTaskToViewModel(task, familyMembersQuery.data?.members ?? [])), [familyMembersQuery.data, tasksQuery.data]);
  const contentQuery = view === "reminders" ? remindersQuery : view === "lists" ? listsQuery : view === "tasks" ? tasksQuery : schoolWeekQuery;
  const refresh = async () => { await Promise.all([familiesQuery.refetch(), contentQuery.refetch()]); };
  return { reminders, lists, tasks, schoolWeek, familyId: familyId ?? null, loading: familiesQuery.isLoading || contentQuery.isLoading, refreshing: familiesQuery.isRefetching || contentQuery.isRefetching, error: familiesQuery.error || contentQuery.error || ((view === "school-week" || view === "tasks") ? familyMembersQuery.error : null), missingContext: !accessToken || (familiesQuery.isSuccess && !familyId), refresh, weekStart, familyMembers: familyMembersQuery.data?.members ?? [] };
}
