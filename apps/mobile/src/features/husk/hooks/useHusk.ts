...
export function useHusk(filter: ReminderFilter = "active", view: HuskView = "reminders") {
  ...
  const schoolWeek = useMemo(() => sortSchoolWeekReminders((schoolWeekQuery.data ?? []).map((item) => mapSchoolWeekReminderToViewModel(item))), [schoolWeekQuery.data]);
  ...
  return { reminders, lists, schoolWeek, familyId: familyId ?? null, loading: familiesQuery.isLoading || contentQuery.isLoading, refreshing: familiesQuery.isRefetching || contentQuery.isRefetching, error: familiesQuery.error || contentQuery.error, missingContext: !accessToken || (familiesQuery.isSuccess && !familyId), refresh };
}
