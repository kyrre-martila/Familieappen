...
function ParticipantAvatar({ participant }: { participant: CalendarEventViewModel["participants"][number] }) { const initial = participant.displayName.trim().charAt(0).toUpperCase() || "?"; return participant.avatarUrl ? <Image source={{ uri: participant.avatarUrl }} style={styles.avatar} accessibilityIgnoresInvertColors /> : <View style={styles.avatarFallback}><AppText style={styles.avatarInitial}>{initial}</AppText></View>; }
...
export function CalendarEventDetails({ event, onEdit, onDuplicate, onDelete, deleting = false, deleteError = null, onResetDeleteError }: { event: CalendarEventViewModel; onEdit?: (path: ReturnType<typeof buildCalendarEventEditPath>) => void; onDuplicate?: () => void; onDelete?: (scope?: CalendarEventDeleteScope) => void | Promise<void>; deleting?: boolean; deleteError?: string | null; onResetDeleteError?: () => void }) {
...
const styles = StyleSheet.create({
  ...
  rowHeader: { flexDirection: "row", gap: theme.spacing.md, alignItems: "center" },
  participantsSection: { gap: theme.spacing.sm },
  participantList: { flexDirection: "row", flexWrap: "wrap", gap: theme.spacing.sm },
  participantPill: { minHeight: 44, flexDirection: "row", alignItems: "center", gap: theme.spacing.xs, borderRadius: theme.radius.pill, backgroundColor: theme.colors.primarySoft, paddingLeft: theme.spacing.xs, paddingRight: theme.spacing.sm },
  participantName: { color: theme.colors.primaryStrong, fontWeight: "800" },
  avatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: theme.colors.surface },
  avatarFallback: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: theme.colors.surface },
  avatarInitial: { color: theme.colors.primaryStrong, fontWeight: "800" },
