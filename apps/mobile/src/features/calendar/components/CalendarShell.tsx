...
export function CalendarShell() {
  ...
  const groupedMonthEvents = useMemo(() => monthDates.map((date) => ({ date, events: calendar.eventsForMonth.filter((event) => event.date === date) })).filter((group) => group.events.length), [calendar.eventsForMonth, monthDates]);
