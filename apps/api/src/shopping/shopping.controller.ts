...
export class ShoppingController {
  ...
  @Get("catalog/categories") @UseGuards(AuthGuard) async getCatalogCategories(@Headers("x-family-id") familyId?: string): Promise<ApiResponse<ShoppingCatalogCategoryDto[]>> { return createApiResponse(await this.shoppingService.getCatalogCategories(familyId)); }
  @Get("catalog/items") @UseGuards(AuthGuard) async getCatalogItems(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId?: string): Promise<ApiResponse<ShoppingCatalogItemDto[]>> { return createApiResponse(await this.shoppingService.getCatalogItems(request.user.id, familyId)); }
  @Get("catalog/search") @UseGuards(AuthGuard) async searchCatalog(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string | undefined, @Query("q") query = ""): Promise<ApiResponse<ShoppingCatalogItemDto[]>> { return createApiResponse(await this.shoppingService.searchCatalog(query, request.user.id, familyId)); }
  ...
  @Post("lists/:listId/invitations/:invitationId/revoke") @UseGuards(AuthGuard) async revokeInvite(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Param("listId") listId: string, @Param("invitationId") invitationId: string): Promise<ApiResponse<ShoppingListInvitationDto>> { return createApiResponse(await this.shoppingService.revokeInvite(request.user.id, requireFamilyId(familyId), listId, invitationId)); }
  @Delete("lists/:listId/collaborators/me") @UseGuards(AuthGuard) async leaveList(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Param("listId") listId: string): Promise<ApiResponse<{ ok: true }>> { await this.shoppingService.removeCollaborator(request.user.id, requireFamilyId(familyId), listId, request.user.id); return createApiResponse({ ok: true }); }
  @Delete("lists/:listId/collaborators/:userId") @UseGuards(AuthGuard) async removeCollaborator(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Param("listId") listId: string, @Param("userId") userId: string): Promise<ApiResponse<{ ok: true }>> { await this.shoppingService.removeCollaborator(request.user.id, requireFamilyId(familyId), listId, userId); return createApiResponse({ ok: true }); }
  @Post("items") @UseGuards(AuthGuard) async addItem(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Query("listId") listId: string | undefined, @Body() body: AddShoppingItemRequestDto): Promise<ApiResponse<AddShoppingItemResponseDto>> { return createApiResponse(await this.shoppingService.addItem(request.user.id, requireFamilyId(familyId), body, listId)); }
  @Patch("items/:itemId") @UseGuards(AuthGuard) async updateOrToggleItem(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Query("listId") listId: string | undefined, @Param("itemId") itemId: string, @Body() body?: UpdateShoppingItemRequestDto): Promise<ApiResponse<ShoppingListItemDto | UpdateShoppingItemResponseDto>> { return createApiResponse(body && Object.keys(body).length > 0 ? await this.shoppingService.updateItem(request.user.id, requireFamilyId(familyId), itemId, body, listId) : await this.shoppingService.toggleItem(request.user.id, requireFamilyId(familyId), itemId, listId)); }
  @Patch("custom-items/:itemId") @UseGuards(AuthGuard) async updateCustomItem(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Param("itemId") itemId: string, @Body() body: UpdateFamilyCustomShoppingItemRequestDto): Promise<ApiResponse<ShoppingCatalogItemDto>> { return createApiResponse(await this.shoppingService.updateCustomItem(request.user.id, requireFamilyId(familyId), itemId, body)); }
  @Delete("custom-items/:itemId") @UseGuards(AuthGuard) async deleteCustomItem(@Req() request: AuthenticatedRequest, @Headers("x-family-id") familyId: string, @Param("itemId") itemId: string): Promise<ApiResponse<{ ok: true }>> { return createApiResponse(await this.shoppingService.deleteCustomItem(request.user.id, requireFamilyId(familyId), itemId)); }
