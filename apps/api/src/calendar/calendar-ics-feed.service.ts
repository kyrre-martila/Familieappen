...
export class CalendarIcsFeedService {
  ...
  async updateFeed(userId: string, familyId: string, feedId: string, input: UpdateCalendarExportFeedRequestDto): Promise<CalendarExportFeedDto> {
    ...
    if (data.scope === "mine" && feed.scope !== "mine") {
      const member = await this.prisma.client.familyMember.findFirst({ where: { familyId, userId }, select: { id: true } });
      if (!member) throw new BadRequestException("The current user must have a family member profile");
      data.mineFamilyMemberId = member.id;
    }
  ...
  private async collectFeedItems(feed: ExportFeedRecord): Promise<IcsItem[]> {
    ...
    const selectedIds = feed.scope === "selectedParticipant" ? feed.selectedMembers?.map(row => row.familyMemberId) ?? [] : feed.scope === "mine" && feed.mineFamilyMemberId ? [feed.mineFamilyMemberId] : [];
    const hasResolvedAudience = feed.scope !== "mine" || selectedIds.length > 0;
    if (feed.includeEvents && hasResolvedAudience) {
