...
export class CalendarService {
  ...
  private async notifyCalendarEvent(
    ...
    try {
      ...
      await this.notificationsService.createNotificationForFamilyMembers({
        ...
        recipientUserIds,
        type,
        title: type === "calendar_event_created" ? "Ny kalenderhendelse" : type === "calendar_event_updated" ? "Kalenderhendelse oppdatert" : "Kalenderhendelse slettet",
        body: `${actorName} ${type === "calendar_event_created" ? "la til" : type === "calendar_event_updated" ? "oppdaterte" : "slettet"} ${event.title}`,
        entityType: "calendar_event",
        entityId: eventId,
        deepLink: `/calendar?view=day&date=${encodeURIComponent(eventDate)}&eventId=${encodeURIComponent(eventId)}${occurrenceDateQuery}`,
        cooldownMinutes: type === "calendar_event_created" ? undefined : 30
      });
    } catch (error) {
  ...
  async deleteOccurrence(userId: string, familyId: string, eventId: string, occurrenceDateValue: string): Promise<void> {
    ...
    await this.notifyCalendarEvent(userId, { ...event, recurringEventId: event.id, occurrenceDate: formatDate(occurrenceDate), isRecurringOccurrence: true }, "calendar_event_deleted");
  }
