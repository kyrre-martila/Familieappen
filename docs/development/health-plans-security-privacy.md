...
Applikasjonens member-removal er hard delete og dermed en autorisasjonsgrense: gamle tokens gir ikke tilgang fordi hvert kall slår opp medlemskapet på nytt. Helseplan lagrer både en nullable aktiv subject-link og `subjectDisplayName`, et immutable snapshot som settes fra medlemmet ved opprettelse. Databasetriggeren avviser en aktiv subject-link til en annen familie. Når subject fjernes, blir DRAFT-planer arkivert og ACTIVE/PAUSED-planer fullført i samme transaksjon som medlemslettingen; terminale planer beholder status. Framtidige PENDING/SNOOZED-occurrences hoppes over, en privacy-safe `STATUS_CHANGED`-historikkrad skrives bare ved faktisk overgang, og subject-linken frikobles. Plan, historikk og snapshot forblir i familien, mens recipient-linker for det slettede medlemmet fjernes med cascade.

Ved kontosletting brukes samme policy for hver familie som har andre medlemmer. En bruker som bare er creator, actor, note-author eller completion-actor terminaliserer ikke planen; eksisterende `SET NULL` og snapshots bevarer historikken. Dersom brukeren er siste medlem, slettes hele familien og hele Helseplan-treet gjennom database-cascade. Det opprettes derfor ingen familieløse Helseplan-rader.
