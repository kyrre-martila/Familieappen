...
`activeLevelStartedAt` and `activeStepStartedAt` are the separate **logical progress clocks** for the current level and step. `pausedAt` is populated exactly while status is `PAUSED`. Resume shifts both logical starts by the pause measured on the Europe/Oslo local calendar. Consequently paused wall-clock time is excluded from duration calculation without accumulating a lossy counter, and a pause across a 23- or 25-hour DST day does not move the step's expiry wall clock by one hour.
...
Notes are append-only in this version: there are no edit or delete endpoints. Plan notes have no target, occurrence comments use the immutable occurrence title and scheduled instant together with the source action's level/step context, and action notes use the targeted action's schedule context. The `Logg` view combines these notes with `HealthPlanHistory`, with bounded queries and explicit plan/family authorization. `COMPLETED` and `ARCHIVED` plans are read-only for every note target as well as definition changes. History labels interpret metadata defensively and fall back to a general update label rather than inventing detail. This family tool is not a medical journal.
...
Each plan is processed in this order: advance every elapsed eligible step, reload the resulting active step and its generation boundary, calculate occurrences, and insert them as one per-plan batch. The rolling window is **four elapsed hours of lookback through fourteen local calendar days ahead**, expressed by exported constants. Its per-plan lower bound is `max(now - four hours, generationNotBefore)`. Four hours therefore recovers an occurrence missed during a short outage only when it belongs to the current context, while intentionally refusing both older downtime backfill and same-day backfill from before activation. The upper boundary preserves the Europe/Oslo wall clock across DST rather than adding fourteen 24-hour periods. Fourteen days provides useful near-future work without unbounded rows. Both lower and upper instants are inclusive. Runs remain bounded by the active step's schedules/actions.
...
- The create flow anchors `INTERVAL_DAYS` to the current local calendar date in `Europe/Oslo`, rather than deriving the date from a UTC ISO timestamp. `autoAdvance` requires an integer `durationDays` of at least one; clearing duration also clears and disables automatic advancement.
...
### V1 notification transport hardening

The notification query uses the schema's `User.notificationPreferences` relation and the occurrence remains the due source. Its inclusive window is `now - 30 minutes <= scheduledAt <= now`; only `ACTIVE` plans and `PENDING`/`SNOOZED` occurrences qualify. A missing preferences row is enabled by default, while `healthPlansEnabled = false`, an unlinked recipient, or a deactivated user suppresses both ledger and delivery. The scheduler is capped at 500 occurrences per minute.

Health-plan delivery explicitly opts into `NotificationsService.createNotificationAndDeliver`; existing notification producers retain their in-app-only behavior. The `Notification` row is committed before transport. Its nullable unique dedupe key combines occurrence, recipient user, and current scheduled instant. A `P2002` identified as the `dedupeKey` race is treated as already processed and does not send; other identifiable unique errors propagate. Snoozing to a new instant creates a new identity, while rerunning the same instant does not.

`PushNotificationService` sends one generic Expo Push API message to each active (`disabledAt IS NULL`) `PushDevice`. Only `Helseplan`, `Du har en planlagt helseplan-hendelse.`, `/health-plans`, and the notification ID leave the server; health-plan and person content never enters the request. Expo's explicit `DeviceNotRegistered` ticket disables that device. Timeouts, HTTP failures, and unknown errors leave devices active and are logged without request content. The ledger is retained when transport fails, so V1 has ledger-level idempotency but no push retry queue or guaranteed transport delivery.

The mobile package includes `expo-notifications`, but currently has no permission/token acquisition or call to `POST /notifications/devices`. Server-side Expo delivery is therefore ready for already registered rows, but end-to-end mobile push is not yet enabled until the client registration flow exists.
