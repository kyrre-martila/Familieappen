...
exports.SHOPPING_CATALOG = exports.SHOPPING_CATEGORIES = exports.SHOPPING_UNITS = void 0;
exports.getShoppingCategoryBySlug = getShoppingCategoryBySlug;
exports.getShoppingCatalogItemsByCategory = getShoppingCatalogItemsByCategory;
exports.normalizeShoppingSearchValue = normalizeShoppingSearchValue;
exports.searchShoppingCatalog = searchShoppingCatalog;
exports.SHOPPING_UNITS = [
    "boks",
    "eske",
    "flaske",
    "glass",
...
function getShoppingCategoryBySlug(slug) {
    return exports.SHOPPING_CATEGORIES.find((category) => category.slug === slug);
}
function getShoppingCatalogItemsByCategory(categorySlug) {
    return exports.SHOPPING_CATALOG.filter((item) => item.categorySlug === categorySlug);
}
function normalizeShoppingSearchValue(value) {
    return value
        .trim()
...
function searchShoppingCatalog(query) {
    const normalizedQuery = normalizeShoppingSearchValue(query);
    if (normalizedQuery.length < 2) {
        return [];
    }
    return exports.SHOPPING_CATALOG.filter((item) => {
        const searchableValues = [item.name, ...item.aliases];
        return searchableValues.some((value) => normalizeShoppingSearchValue(value).includes(normalizedQuery));
    });
}
