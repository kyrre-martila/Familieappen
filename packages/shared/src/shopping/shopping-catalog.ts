...
export interface ShoppingCategory {
  ...
  sortOrder: number;
  /**
   * Optional legacy source list used by catalog completeness checks.
   * When populated, every item must have a matching SHOPPING_CATALOG entry.
   */
...
export interface ShoppingCatalogCompletenessIssue {
  type: "missing-catalog-item" | "unknown-category-slug";
  categorySlug: string;
  itemName?: string;
}

export function getShoppingCatalogCompletenessIssues(): ShoppingCatalogCompletenessIssue[] {
  const categorySlugs = new Set(SHOPPING_CATEGORIES.map((category) => category.slug));
  const catalogKeys = new Set(
    SHOPPING_CATALOG.map(
      (item) => `${item.categorySlug}:${normalizeShoppingSearchValue(item.name)}`,
    ),
  );
  const issues: ShoppingCatalogCompletenessIssue[] = [];

  for (const category of SHOPPING_CATEGORIES) {
    for (const itemName of (category as ShoppingCategory).items ?? []) {
      const itemKey = `${category.slug}:${normalizeShoppingSearchValue(itemName)}`;

      if (!catalogKeys.has(itemKey)) {
        issues.push({
          type: "missing-catalog-item",
...
export function assertShoppingCatalogCompleteness(): void {
  const issues = getShoppingCatalogCompletenessIssues();
  ...
  throw new Error(
    `Shopping catalog completeness check failed: ${issues
      .map((issue) =>
        issue.type === "missing-catalog-item"
          ? `${issue.itemName} is missing from SHOPPING_CATALOG for ${issue.categorySlug}`
          : `${issue.itemName} uses unknown categorySlug ${issue.categorySlug}`,
      )
      .join("; ")}`,
  );
}
